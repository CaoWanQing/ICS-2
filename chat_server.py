...
#                 BLACKJACK
#==============================================================================
            elif msg["action"] == "blackjack":
                from_name = self.logged_sock2name[from_sock]
                to_name = msg["target"]
                to_sock = self.logged_name2sock[to_name]
                from_sock = self.logged_name2sock[from_name]
                if to_name == from_name:
                    msg = json.dumps({"action": "blackjack", "status": "self"})
                elif self.group.is_member(to_name):
                    msg = json.dumps({"action":"connect", "status":"success"})
                    mysend(to_sock, json.dumps({"action":"blackjack", "status":"success", "from":from_name}))
                else:
                    msg = json.dumps({"action":"blackjack", "status": "no-user"})
                mysend(from_sock, msg)
            elif msg["action"] == "sendscore":
                #print("sendscore reached server!")
                from_name = self.logged_sock2name[from_sock]
                to_name = msg["target"]
                to_sock = self.logged_name2sock[to_name]
                score = msg["score"]
                msg = json.dumps({"action":"sendscore", "score":score, "active": msg["active"]})
                mysend(to_sock, msg)
...
#==============================================================================
            elif msg["action"] == "bjdisconnect":
                from_name = self.logged_sock2name[from_sock]
                to_name = msg["peer"]
                to_sock = self.logged_name2sock[to_name]
                mysend(to_sock, json.dumps({"action":"bjdisconnect"}))
